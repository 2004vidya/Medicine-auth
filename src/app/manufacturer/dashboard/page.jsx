...
const ManufacturerDashboard = () => {
  ...
  // New state for ingredients and dosage
  ...
  const handleAddMedicine = async (e) => {
    ...
    const newMedicine = {
      ...
      batchNo: batchNumber,
      expiry: expiryDate,
      // Add the new fields to the medicine object
      ingredients: ingredients,
      dosageForm: dosage, // Using dosage for dosageForm
      strength: "", // Empty for now since you don't have strength state
  ...
  const handleNameBlur = async () => {
    ...
    try {
      const res = await fetch(`/api/medicine/lookup?name=${medicineName}`);
      const data = await res.json();

      setIngredients(data.ingredients || "");
      setDosage(
        [data.dosageForm, data.strength].filter(Boolean).join(" ") || ""
      );
    } catch (error) {
